...
- Security review: a first pass is done. Phase E now fails closed in production on a missing or default session secret and a missing mail provider, and every server-side URL fetch goes through an SSRF guard (`app/net/guard.py`) that blocks non-public addresses and re-validates each redirect hop. `/auth/request` is rate limited per email (a sliding window of five links per fifteen minutes). Sign-out revokes sessions server-side by bumping the user's session epoch, which every issued token carries.
...
Still open: per-IP rate limiting, closing the DNS-rebinding window, and the dev-tooling major upgrades that clear the npm advisories.
