...
| B | 🔨 Gates + anonymous persistence + merge. Gates form, rubric merge, and versioned server-side persistence (anonymous-id keyed) done; magic-link account claim delivered in Phase E. |
| C | 🔨 Scoring service. Deterministic core + style-affinity done; value and age categories scored from facts (age from year_built via configurable bands, with a systems due-diligence prompt for older homes); scoring-contract.md written; Claude vision analyzer built (gated on ANTHROPIC_API_KEY) with the two-tier pass wired: photos resized to a ~1300px long edge and a cheap triage model dedups near-duplicate rooms before the strong analysis pass. Live cost/latency check still needs a real key. |
...
| E | 🔨 Accounts + magic-link claim. Passwordless one-time link (email sender seam gated on RESEND_API_KEY, console sender otherwise returns a dev_link); verify claims the anonymous rubric onto the account (compose-forward when the email already has one) and issues a signed session; login_tokens table, /auth routes, and a web account bar built. Live email send needs a provider key. |
